...
public class ClassesRegistration {
    public void loadListeners(String packageName) {
        for (Class<?> clazz : getClassesInPackage(packageName)) {
            if (isListener(clazz)) {
                try {
                    DbzMain.instance.getServer().getPluginManager().registerEvents((Listener) clazz.newInstance(), DbzMain.instance);
    ...
    public Collection<Class<?>> getClassesInPackage(String packageName) {
        ...
        CodeSource codeSource = DbzMain.instance.getClass().getProtectionDomain().getCodeSource();
