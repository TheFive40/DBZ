...
public class CommandFramework implements CommandExecutor {
    ...
    private final DbzMain plugin;

    public CommandFramework(DbzMain plugin) {
        this.plugin = plugin;
        try {
            map = Bukkit.getServer().getCommandMap();
        } catch (NoSuchMethodError e) {
            try {
                Field field = plugin.getServer().getPluginManager().getClass()
                        .getDeclaredField("commandMap");
                field.setAccessible(true);
                map = (CommandMap) field.get(plugin.getServer().getPluginManager());
    ...
    public boolean handleCommand(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args) {
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (command.isAdminOnly() && !sender.hasPermission("dbcplugin.admin")) {
                    sender.sendMessage(ChatColor.RED + "Solo Ops pueden ejecutar este comando.");
                    return true;
                }
                ...
                if (command.inGameOnly() && !(sender instanceof Player)) {
                    sender.sendMessage(ChatColor.RED + "Este comando solo se puede usar in game");
                    return true;
                }
