...
public class BoosterUtils {
    ...
    public static String formatPercentage(double multiplier) {
        ...
        if (percentage > 0) return "§a+" + percentage + "%";
        else if (percentage < 0) return "§c" + percentage + "%";
        else return "§60%";
    }
    ...
    public static String formatTime(long seconds) {
        ...
        if (hours > 0) return String.format("§6%dh %dm %ds", hours, minutes, secs);
        else if (minutes > 0) return String.format("§6%dm %ds", minutes, secs);
        else return String.format("§6%ds", secs);
    }
