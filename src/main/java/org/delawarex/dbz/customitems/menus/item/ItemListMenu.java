...
public class ItemListMenu extends Menu {
    ...
    public ItemListMenu(int page) { this.page = page; }

    @Override protected String getTitle() { return "&c&lItems Custom — Pág. " + page; }
    ...
    protected void buildContents(Player player) {
        ...
        for (int i = start; i < end; i++) {
            ...
            // Use the real ItemStack so the GUI shows the actual item
            ItemStack display = CustomItemManager.getInstance().buildItemStack(ci);

            // Append info lore without modifying the stored item
            ...
            if (meta != null) {
                ...
                lore.add(ChatColor.DARK_GRAY + "Material: " + ci.getMaterial());
                lore.add("");
            ...
            set(slot, displayClone, e -> new ItemEditMenu(id).open(player));
        }

        // Navigation
        if (page > 1)
            set(45, navButton("◀ Anterior", true), e -> new ItemListMenu(page - 1).open(player));

        set(49, item(org.bukkit.Material.BOOK,
                "&f&lPágina " + page + "/" + Math.max(1, pages)));

        if (page < pages)
            set(53, navButton("Siguiente ▶", true), e -> new ItemListMenu(page + 1).open(player));

        set(48, back(), e -> new ItemMainMenu().open(player));
    }
